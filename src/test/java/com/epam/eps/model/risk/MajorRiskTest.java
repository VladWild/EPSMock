...
public class MajorRiskTest {
	...
	public void hasRiskZoneAnnotationTest() {
		assertTrue(MajorRisk.class.isAnnotationPresent(RiskZone.class));
	}
	...
	public void hasOnlyOneMinAnnotationTest() {
		...
		for (Field field : MajorRisk.class.getDeclaredFields()) {
	...
	public void hasOnlyOneAddAnotherOneGroupAnnotationTest() {
		...
		for (Method method : MajorRisk.class.getDeclaredMethods()) {
	...
	public void beanInContextTest() throws Exception {
		EpsResourceBundleAnnotationContext epsResourceBundleAnnotationContext = new EpsResourceBundleAnnotationContext(
				"config");
		Object epsBean;
		final String id = MajorRisk.class.isAnnotationPresent(RiskZone.class)
				? MajorRisk.class.getAnnotation(RiskZone.class).id() : "";
	...
	public void addAnotherOneGroupTest() throws IllegalAccessException,
		...
		Risk majorRiskMock = new MajorRisk();
		boolean groupAdded = false;
		for (Method method : MajorRisk.class.getDeclaredMethods()) {
			if (method.isAnnotationPresent(AddAnotherOneGroup.class)) {
				method.invoke(majorRiskMock, groupMock);
				groupAdded = majorRiskMock.getGroups().contains(groupMock);
			}
		}
	...
	public void getMaxTest() throws IllegalAccessException,
		...
		for (Field field : MajorRisk.class.getDeclaredFields()) {
			if (field.isAnnotationPresent(Max.class)) {
				...
				field.set(majorRiskMock, EXPECTED_MAX_VALUE);
				actualMax = majorRiskMock.getMax();
			}
		}
	...
	public void getMinTest() throws IllegalAccessException,
		...
		for (Field field : MajorRisk.class.getDeclaredFields()) {
			if (field.isAnnotationPresent(Min.class)) {
				field.setAccessible(true);
				field.set(majorRiskMock, ACTUAL_MIN_VALUE);
				actualMin = majorRiskMock.getMin();
			}
		}
